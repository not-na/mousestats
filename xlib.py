...
# From https://github.com/MBaltz/kbdcounter/blob/master/src/xlib.py
...
class XEvent(object):
    ...
    def __str__(self):
        return "type:%s scancode:%s code:%s value:%s" % (
            self._type,
            self._scancode,
            self._code,
            self._value,
...
class XEvents(threading.Thread):
    ...
    _butn_to_code = collections.defaultdict(
        lambda: "BTN_DUNNO",
        [
            (1, "BTN_LEFT"),
            (2, "BTN_MIDDLE"),
    ...
    def _handle_mouse(self, event, value):
        ...
        """
        if value == 2:
            self.events.append(XEvent("EV_MOV", 0, 0, (event.root_x, event.root_y)))
            ...
            if event.detail == 5:
                ...
                value = 1
            self.events.append(
                XEvent("EV_REL", 0, XEvents._butn_to_code[event.detail], value)
            )
        else:
            self.events.append(
                XEvent("EV_KEY", 0, XEvents._butn_to_code[event.detail], value)
            )
